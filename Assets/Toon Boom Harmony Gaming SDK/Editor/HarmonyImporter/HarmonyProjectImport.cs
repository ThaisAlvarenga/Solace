...
namespace ToonBoom.Harmony
{
    ...
    public class HarmonyProjectImport : ScriptableObject
    {
        ...
        public bool SkipPaletteHack = false; 
        ...
        public void DoImport()
        {
            try
            {
                ...
                //Make project file
                var project = CreateProject(bytes);
                ...
                // capy addition--this might be the old/existing project reference if it's already on disk, so catch return
                project = SaveProjectAsset(project);
        ...
        private HarmonyProjectBinary SaveProjectAsset(HarmonyProjectBinary project)
        {
            if (!Directory.Exists(ProjectFolder))
                Directory.CreateDirectory(ProjectFolder);

            // capy addition--inline replace of existing project if it's already there to keep loaded objects intact
            var updatedProject = CreateOrReplaceAsset(project, OutputPath);

            return updatedProject; 
        }

        private T CreateOrReplaceAsset<T>(T asset, string path) where T : UnityEngine.Object
        {
        ...
        private void ReplaceDirectoryFiltered(string sourceDir, string destinationDir, bool recursive, string excludeExtention)
        {
            destinationDir = conditionPath(destinationDir);
            sourceDir = conditionPath(sourceDir);
            
            var dir = new DirectoryInfo(destinationDir);
            if (dir.Exists)
            {
                FileUtil.DeleteFileOrDirectory(destinationDir);
            }
            
            CopyDirectoryFiltered(sourceDir, destinationDir, recursive, excludeExtention);
        }
        private void CopyDirectoryFiltered(string sourceDir, string destinationDir, bool recursive, string excludeExtention)
        {
            destinationDir = conditionPath(destinationDir);
            sourceDir = conditionPath(sourceDir);
            ...
            {
                if (!file.Extension.Equals(excludeExtention, StringComparison.InvariantCultureIgnoreCase))
                {
            ...
            if (recursive)
            {
                foreach (DirectoryInfo subDir in dirs)
                {
                    ...
                    CopyDirectoryFiltered(subDir.FullName, newDestinationDir, recursive, excludeExtention);
                }
            }
        }
        ...
        private void ImportSprites(HarmonyProject project)
        {
            ...
            if (useSprites)
            {
                ReplaceDirectoryFiltered(HarmonySpritesPath, ProjectFolder + "/sprites/", true, ".sprite");
                //FileUtil.ReplaceDirectory(HarmonySpritesPath, ProjectFolder + "/sprites/");
            }
            else
            {
                ReplaceDirectoryFiltered(HarmonySpritesheetPath, ProjectFolder + "/spriteSheets/", true, ".sprite");
                //FileUtil.ReplaceDirectory(HarmonySpritesheetPath, ProjectFolder + "/spriteSheets/");
            }

            //Pair atlases to resolutions
            for (int i = 0, len = project.SpriteSheets.Count; i < len; i++)
            {
                Spritesheet resolution = project.SpriteSheets[i];
        ...
        private GameObject BuildOrUpdatePrefabAsset(HarmonyProjectBinary project)
        {
            ...
            // capy addition:  set harmony renderer dirty if we have one too
            if (harmonyRenderer)
            {
	            EditorUtility.SetDirty(harmonyRenderer);
            }
            
            // Start/Stop asset editing to make sure the clone is destroyed before the saved/updated prefab is imported
            // - no need to asset post process the clone that is about to be destroyed
            AssetDatabase.StartAssetEditing();
            string prefabPath = ProjectFolder + "/" + ProjectName + ".prefab"; 
            ExistingPrefab = PrefabUtility.SaveAsPrefabAsset(prefab, prefabPath);
